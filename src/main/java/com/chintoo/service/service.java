...
public class service {
	...
	{		
		...
		myLikes = (List<MyLike>) likeRepository.save(myLikes);
	...
	{
		FacebookClient client = new DefaultFacebookClient(accessToken, Version.VERSION_3_2);
		Connection<LikeItem> connectionLikes = client.fetchConnection(postUniqueId + "/likes", LikeItem.class, Parameter.with("limit", 10));
		...
		for(List<LikeItem> likeItems : connectionLikes){
			for(LikeItem item : likeItems){
				...
				myLike = likeRepository.save(myLike);
